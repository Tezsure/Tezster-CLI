...
var helpData =
  "Usage: tezster [command] [optional parameters].....\n" +
...
function transferAmount(args) {
  ...
  return eztz.rpc
    .transfer(from, keys, to, amount, fees, undefined, 10600)
    .then(function(r) {
...
function createAccount(args) {
  var pkh = args[0],
    f;
  if (findKeyObj(config.accounts, args[1]))
    return console.log(outputError("That account name is already in use"));
  if ((f = findKeyObj(config.identities, pkh))) {
    return eztz.rpc
      .account(f, parseFloat(args[2]), true, true, f.pkh, 1400)
      .then(function(r) {
...
async function deployContract(contractLabel, contractPath, initValue, account) {
  ...
  if (contractObj) {
    return outputError(
      `This contract label is already in use. Please use a different one.`
    );
  }
  ...
  try {
    ...
    if (result.results) {
      switch (result.results.contents[0].metadata.operation_result.status) {
        case "applied":
          let opHash = result.operationGroupID.slice(
            1,
            result.operationGroupID.length - 2
          );
          opHash = eztz.contract.hash(opHash);
          addContract(contractLabel, opHash, keys.pkh);
          return output(
            `contract ${contractLabel} has been deployed at ${opHash}`
          );

        case "failed":
        default:
          return outputError(
            `Contract deployment has failed : ${JSON.stringify(
    ...
    return outputError(
      `Contract deployment has failed : ${JSON.stringify(result)}`
    );
  } catch (error) {
...
async function invokeContract(contract, argument, account) {
  ...
  if (!contractAddress) {
    return outputError(
      `couldn't find the contract, please make sure contract label or address is correct!`
    );
  }
  ...
  try {
    ...
    if (result.results) {
      switch (result.results.contents[0].metadata.operation_result.status) {
        ...
        default:
          return outputError(
            `Contract calling has failed : ${JSON.stringify(
              result.results.contents[0].metadata.operation_result
            )}`
          );
      }
    ...
    return outputError(
      `Contract calling has failed : ${JSON.stringify(result)}`
    );
  } catch (error) {
    return outputError(error);
  }
...
function restoreAlphanetAccount(accountLabel, accountFilePath) {
  ...
  try {
    ...
    if (!mnemonic || !email || !password) {
      return outputError(
        ` occured while restroing account : invalid JSON file`
      );
    }
    ...
    */
    ...
    return output(
      `successfully restored testnet faucet account: ${accountLabel}-${alphakeys.pkh}`
    );
  } catch (error) {
...
async function activateAlphanetAccount(account) {
  ...
  try {
    ...
    return output(
      `successfully activated testnet faucet account: ${keys.label} : ${keys.pkh} \n with tx hash: ${activationResult}`
    );
  } catch (error) {
