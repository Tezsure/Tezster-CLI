...
class Contracts{
  ...
  async deployContract() {
      var args = process.argv.slice(3);
          ...
          console.log(result);
          console.log(Helper.outputInfo(`If you're using babylonnet node, use https://babylonnet.tzstats.com to check contract/transactions`));
  }
  ...
  async getStorage() {
      ...
      let result = await this.__getStorage(args[0]);
      console.log(result);
  }

  async addContract() {
      var args = process.argv.slice(3);  
      if (args.length < 2) return console.log(Helper.outputError("Incorrect usage - tezster add-contract <label> <Address>"));
      await this.loadTezsterConfig();
  ...
  addContractToConfig(contractLabel, contractAddr) {
      ...
      this.__addContract(contractLabel, contractAddr, '');
  }

  __addContract(label, opHash, pkh) {
