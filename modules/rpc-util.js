...
class RpcRequest {
    ...
    static fetchBlockDetailsForRemoteNodes(provider) {
        provider = provider.includes('mainnet') ? 'https://api.tzstats.com' : provider.includes('edo') ? `https://api.${TZSTATS_NODE_TYPE.TESTNET}.tzstats.com`: `https://api.${TZSTATS_NODE_TYPE.TESTNET}.tzstats.com`;
        return Promise.all(
